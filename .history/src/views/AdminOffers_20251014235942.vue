<template>
    <div class="skill-offer-platform">
      ...
      <div class="search-report-container">
        ...
        <button class="reported-posts-btn" @click="showReportedPosts = !showReportedPosts" :class="{ 'offer-posts-btn': showReportedPosts }">
          <span class="material-icons" v-if="!showReportedPosts">flag</span>
      ...
      <div class="header-container">
        <h1 class="offers-header">{{ showReportedPosts ? 'Reported Posts' : 'Offers' }}</h1>
        <div class="offer-count">
          {{ showReportedPosts ? 'Total Reported:' : 'Total Offers:' }} <span>{{ showReportedPosts ? reportedPosts.length : filteredPosts.length }}</span>
        </div>
      </div>
  
      <!-- Regular Posts Container -->
      <div v-if="!showReportedPosts" class="posts-container">
        <div 
        ...
        >
          ...
          <div class="post-content">
            <h2 class="offer-title">{{ post.skills }}</h2>
          ...
          <div class="action-buttons">
            <button class="btn delete-btn" @click="openDeleteModal(post)">
      ...
      <div v-else class="reported-posts-container">
        <div class="posts-container">
          <div 
            v-for="post in reportedPosts" 
            :key="post.id" 
            class="post-card reported-post"
          >
            <!-- User Info -->
            <div class="user-info">
              <img :src="post.userPicture" :alt="post.userName" class="user-picture">
            ...
            <div class="action-buttons">
              <button class="btn keep-btn" @click="keepPost(post)">
                <span class="material-icons">check_circle</span>
                Keep
              </button>
              <button class="btn remove-btn" @click="openRemoveModal(post)">
      ...
      <div v-if="showDeleteModal" class="modal-overlay delete-overlay" @click="closeDeleteModal">
      ...
      <div v-if="showRemoveModal" class="modal-overlay remove-overlay" @click="closeRemoveModal">
        <div class="modal-content remove-modal" @click.stop>
          <div class="remove-header">
            <h2>Remove Reported Post</h2>
          </div>
          
          <div class="remove-content">
            <div class="remove-warning-icon">
              <span class="material-icons">warning</span>
            </div>
          ...
          <div class="remove-actions">
            <button class="btn confirm-remove-btn" @click="confirmRemove">
              Confirm Remove
            </button>
            <button class="btn cancel-btn" @click="closeRemoveModal">
              Cancel
            </button>
          </div>
        </div>
  ...
  const showReportedPosts = ref(false)
  ...
  const confirmDelete = () => {
    if (postToDelete.value) {
      ...
      // Remove from reported posts if it exists there
      const reportedIndex = reportedPosts.value.findIndex(p => p.id === postToDelete.value.id)
      if (reportedIndex !== -1) {
        reportedPosts.value.splice(reportedIndex, 1)
      }
  ...
  const closeRemoveModal = () => {
    ...
    postToRemove.value = null
  }
  
  // Confirm remove
  const confirmRemove = () => {
    if (postToRemove.value) {
      // Remove from reported posts
      const reportedIndex = reportedPosts.value.findIndex(p => p.id === postToRemove.value.id)
      if (reportedIndex !== -1) {
        reportedPosts.value.splice(reportedIndex, 1)
      }
      
      // Remove from regular posts if it exists there
      const index = posts.value.findIndex(p => p.id === postToRemove.value.id)
      if (index !== -1) {
        posts.value.splice(index, 1)
      }
      
      console.log('Post removed:', postToRemove.value)
    }
    
    closeRemoveModal()
  }
  
  // Keep post (remove from reported posts)
  const keepPost = (post) => {
    const index = reportedPosts.value.findIndex(p => p.id === post.id)
    if (index !== -1) {
      reportedPosts.value.splice(index, 1)
      console.log('Post kept:', post)
    }
  }
  
  // Remove post (delete from both lists)
  const removePost = (post) => {
    // Remove from reported posts
    const reportedIndex = reportedPosts.value.findIndex(p => p.id === post.id)
    if (reportedIndex !== -1) {
      reportedPosts.value.splice(reportedIndex, 1)
    }
  ...
  /* Search Bar and Reported Posts Button Container */
  ...
  .reported-posts-btn:hover {
    background: #c0392b;
  }
  
  .reported-posts-btn .material-icons {
    margin-right: 8px;
  }
  ...
  .offer-posts-btn:hover {
    background: #5a7284;
  }
  
  .offer-posts-btn .material-icons {
    display: none;
  }
  ...
  /* Delete and Remove Modals - Higher z-index */
  .delete-overlay,
  .remove-overlay {
    z-index: 2000;
  }
  ...
  .remove-warning-icon .material-icons {
    font-size: 3rem;
    color: #e74c3c;
  }
