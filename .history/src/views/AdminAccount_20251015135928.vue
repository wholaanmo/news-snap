...
const confirmDelete = () => {
  if (postToDelete.value) {
    ...
    if (postToDelete.value.skills) {
      // It's a skill offer post
      currentProfile.value.skillOffers = currentProfile.value.skillOffers.filter(
        offer => offer.id !== postToDelete.value.id
      )
    } else {
      // It's a skill request post
      currentProfile.value.skillRequests = currentProfile.value.skillRequests.filter(
        request => request.id !== postToDelete.value.id
      )
    }
    console.log('Deleted post:', postToDelete.value)
  }
  closeDeleteModal()
}
</script>
