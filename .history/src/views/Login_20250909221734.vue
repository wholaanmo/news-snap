...
body {
  ...
  height: 550px;
  border-radius: 20px;
  overflow: hidden; /* so the inner area is clipped */
  box-shadow: 0 10px 25px rgba(0,0,0,0.15), 0 5px 10px rgba(0,0,0,0.05);
  z-index: 0;
}

/* gradient border using ::before; the inner content sits above it */
.container::before {
  content: "";
  position: absolute;
  ...
  -webkit-mask-composite: xor;       /* webkit only */
  mask-composite: exclude;           /* not supported everywhere */
  z-index: -1;
}

/* inner content styling (same look as before) */
.container > * {
  position: relative;
  width: 100%;
  height: 100%;
